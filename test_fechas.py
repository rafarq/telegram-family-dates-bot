...
from bot import _es_dia_de_celebracion, _persona_de, elegir_tarjeta, generar_html_felicitacion
...
def test_generar_html_felicitacion_incluye_nombre_y_tipo():
    coincidentes = [
        {"tipo": "recurrente", "nombre": "Cumpleaños de Ana", "dia": 20, "mes": 8},
    ]
    estado = {}
    html = generar_html_felicitacion(coincidentes, date(2026, 8, 20), estado=estado)
    assert "Ana" in html
    assert "CUMPLEAÑOS" in html
    assert "20 de agosto" in html
    assert "{{" not in html
    # La tarjeta elegida queda registrada en la rotación
    assert len(estado["tarjetas_usadas"]["cumple"]) == 1


def test_elegir_tarjeta_no_repite_hasta_agotar():
    estado = {}
    elegidas = set()
    for _ in range(10):
        archivo, _ = elegir_tarjeta("cumple", estado)
        elegidas.add(archivo)
    assert len(elegidas) == 10  # las 10 distintas
    # La 11ª reinicia el ciclo (puede repetir)
    archivo, _ = elegir_tarjeta("cumple", estado)
    assert archivo in elegidas
